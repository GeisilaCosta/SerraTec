...
public class ControllerExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleEmailException(EmailException e) {
		...
		ErroResposta er = new ErroResposta(http.value(), e.getMessage(), LocalDateTime.now(), erros);
		return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(er);
	}

	@ExceptionHandler(ConfirmarSenhaException.class) // excecção de E-mail
	protected ResponseEntity<Object> handleConfirmarSenhaException(ConfirmarSenhaException e) {
		List<String> erros = new ArrayList<>();
		erros.add(e.getMessage());
		HttpStatus http = HttpStatus.BAD_REQUEST;
		ErroResposta er = new ErroResposta(http.value(), e.getMessage(), LocalDateTime.now(), erros);
